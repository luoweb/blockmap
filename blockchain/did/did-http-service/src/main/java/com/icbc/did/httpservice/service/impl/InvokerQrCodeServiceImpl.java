...
public class InvokerQrCodeServiceImpl extends BaseService implements InvokerQrCodeService {
    ...
    private static final String SERVER_ADDRESS = PropertiesUtil.getProperty("server.address");
    private static final String HTTP_TYPE = PropertiesUtil.getProperty("http.type");
    ...
    public HttpResponseData<Object> createQrCodeInvoke(InputArg createQrCodeFuncArgs) {
        QrCode qrCode = null;
        try {
            qrCode = (QrCode) JsonUtil.jsonStrToObj(new QrCode(), createQrCodeFuncArgs.getFunctionArg());
        } catch (Exception e) {
            // The input credential is a json, so keep moving down.
            logger.info("Detected Portable-Json format qrCode, continuing..");
        }

        Map<String, Object> credMap = (Map<String, Object>) JsonUtil.jsonStrToObj(new HashMap<String, Object>(),createQrCodeFuncArgs.getFunctionArg());

        if (qrCode == null) {
            try {
                credMap = JsonUtil.reformatCredentialPojoToJson(credMap);
                qrCode = (QrCode) JsonUtil.jsonStrToObj(new QrCode(), JsonUtil.mapToCompactJson(credMap));
            } catch (Exception e) {
        ...
        try {
            ...
            String netType = String.valueOf(credMap.get("netType"));
            String url = HTTP_TYPE + "://"+SERVER_ADDRESS+"/did/api/verify?args=";
