...
import com.blockmap.did.core.constant.*;
...
import com.blockmap.did.core.protocol.cpt.Cpt111;
...
import com.blockmap.did.core.util.*;
import com.blockmap.did.core.constant.*;
import com.blockmap.did.core.protocol.base.*;
...
public class CredentialPojoServiceImpl  implements CredentialPojoService {
    ...
    private static void addSelectSalt(
        ...
        for (Map.Entry<String, Object> entry : disclosureMap.entrySet()) {
            ...
            if (value == null) {
                throw new DidBaseException(ErrorCode.CREDENTIAL_POLICY_DISCLOSUREVALUE_ILLEGAL);
    ...
    private static ErrorCode verifySingleSignedCredential(
        ...
        if (StringUtils.isEmpty(publicKey)) {
            ...
            ResponseData<DidDocument> innerResponseData =
                didService.getDidDocument(issuerWeid);
            if (innerResponseData.getErrorCode() != ErrorCode.SUCCESS.getCode()) {
                ...
                DidDocument weIdDocument = innerResponseData.getResult();
                return DataToolUtils
                    .verifySignatureFromDid(rawData, credential.getSignature(), weIdDocument);
            }
        } else {
            boolean result;
            try {
    ...
    private static UserResult makeCredential(
       ...
       DidAuthentication didAuthentication) {
        ...
        String id = new StringBuffer().append(didAuthentication.getDid()).append("_")
    ...
    public ResponseData<CredentialPojo> createCredential(CreateCredentialPojoArgs args) {
        ...
        try {
            ...
            if (!DidUtils.validatePrivateKeyDidMatches(
                args.getDidAuthentication().getDidPrivateKey(),
                args.getIssuer())) {
                logger.error("Create Credential, private key does not match the current did.");
                return new ResponseData<>(null, ErrorCode.DID_PRIVATEKEY_DOES_NOT_MATCH);
            }
            ...
            String privateKey = args.getDidAuthentication().getDidPrivateKey().getPrivateKey();
            ...
            String weIdPublicKeyId = args.getDidAuthentication().getDidPublicKeyId();
    ...
    public ResponseData<CredentialPojo> addSignature(
        ...
        if (credentialList == null || credentialList.size() == 0
            || CredentialPojoUtils.isDidAuthenticationValid(callerAuth) != ErrorCode.SUCCESS) {
        ...
        String privateKey = callerAuth.getDidPrivateKey().getPrivateKey();
        ECKeyPair keyPair = ECKeyPair.create(new BigInteger(privateKey));
        String keyDid = DidUtils
            .convertAddressToDid(Keys.getAddress(keyPair));
        ...
        String weIdPublicKeyId = callerAuth.getDidPublicKeyId();
        result.putProofValue(ParamKeyConstant.PROOF_CREATOR, weIdPublicKeyId);
    ...
    public ResponseData<Boolean> verify(String issuerDid, CredentialPojo credential) {
        ...
        if (!StringUtils.equals(issuerDid, issuerId)) {
            logger.error("[verify] The input issuer did is not match the credential's.");
    ...
    public ResponseData<Boolean> verify(
        ...
        ErrorCode errorCode =
            checkInputArgs(presenterDid, presentationPolicyE, challenge, presentationE);
        if (errorCode.getCode() != ErrorCode.SUCCESS.getCode()) {
            ...
            for (CredentialPojo credential : credentialList) {
                ...
                if (claimPolicy != null) {
                    ErrorCode verifypolicyResult = this
                        .verifyPolicy(credential, claimPolicy, presenterDid);
    ...
    private ErrorCode checkInputArgs(
        ...
        if (StringUtils.isBlank(presenterDid)
            || challenge == null
        ...
        //verify presenterDid
        if (StringUtils.isNotBlank(challenge.getDid())
            && !presenterDid.equals(challenge.getDid())) {
            logger.error("[verify] The input issuer did is not match the presentian's.");
            return ErrorCode.CREDENTIAL_PRESENTERDID_NOTMATCH;
        }
        ...
        DidDocument didDocument = didService.getDidDocument(presenterDid).getResult();
        if (didDocument == null) {
            logger.error(
                "[verify]presentation verify failed, because the presenter did :{} "
                    + "does not exist.",
                presenterDid);
        ...
        errorCode =
            DataToolUtils
                .verifySignatureFromDid(presentationE.toRawData(), signature, didDocument);
        if (errorCode.getCode() != ErrorCode.SUCCESS.getCode()) {
            logger.error(
    ...
    private ErrorCode verifyPolicy(CredentialPojo credentialPojo, ClaimPolicy claimPolicy,
        ...
        if (idValue != null) {
            Object did = credentialPojo.getClaim().get("id");
            if (StringUtils.equals(String.valueOf(idValue), DISCLOSED)) {
                if (!StringUtils.equals(String.valueOf(did), presenterDid)) {
                    logger.error(
                        "[verifyPolicy] the presenter did->{} of presentation does not "
                            + "match the credential's ->{}. ",
                        presenterDid,
                        did);
                    return ErrorCode.PRESENTATION_DID_CREDENTIAL_DID_MISMATCH;
                }
                ...
                logger.error(
                    ...
                    presenterDid,
                    did);
                return ErrorCode.PRESENTATION_CREDENTIAL_CLAIM_DID_NOT_EXIST;
            }
        }
    ...
    public ResponseData<PresentationE> createPresentation(
        ...
        try {
            ...
            presentation.getType().add(DidConstant.DEFAULT_PRESENTATION_TYPE);
            // 处理proof数据
    ...
    private ErrorCode validateCreateArgs(
        ...
        if (weIdAuthentication.getDidPrivateKey() == null
            || !DidUtils.validatePrivateKeyDidMatches(
            weIdAuthentication.getDidPrivateKey(), weIdAuthentication.getDid())) {
            return ErrorCode.DID_PRIVATEKEY_DOES_NOT_MATCH;
        }
        if (!StringUtils.isBlank(challenge.getDid())
            && !challenge.getDid().equals(weIdAuthentication.getDid())) {
            return ErrorCode.PRESENTATION_CHALLENGE_DID_MISMATCH;
        }
        if (StringUtils.isBlank(weIdAuthentication.getDidPublicKeyId())) {
            return ErrorCode.PRESENTATION_DID_PUBLICKEY_ID_INVALID;
        }
    ...
    private ErrorCode validateClaimPolicy(
        ...
        if (!DidUtils.isDidValid(presentationPolicyE.getPolicyPublisherDid())) {
            return ErrorCode.PRESENTATION_POLICY_PUBLISHER_DID_INVALID;
        }
        ResponseData<Boolean> weIdRes = didService
            .isDidExist(presentationPolicyE.getPolicyPublisherDid());
        if (ErrorCode.SUCCESS.getCode() != weIdRes.getErrorCode() || !weIdRes.getResult()) {
            return ErrorCode.PRESENTATION_POLICY_PUBLISHER_DID_NOT_EXIST;
        }
    ...
    private void generatePresentationProof(
        ...
        String weIdPublicKeyId = weIdAuthentication.getDidPublicKeyId();
        ...
        String signature =
            DataToolUtils.sign(
                ...
                weIdAuthentication.getDidPrivateKey().getPrivateKey()
            );
    ...
    public ResponseData<CredentialPojo> createTrustedTimestamp(
        ...
        DidAuthentication weIdAuthentication) {
        if (credentialList == null || credentialList.size() == 0
            || CredentialPojoUtils.isDidAuthenticationValid(weIdAuthentication)
            != ErrorCode.SUCCESS) {
            return new ResponseData<>(null, ErrorCode.ILLEGAL_INPUT);
        }
        ...
        String privateKey = weIdAuthentication.getDidPrivateKey().getPrivateKey();
        ECKeyPair keyPair = ECKeyPair.create(new BigInteger(privateKey));
        String keyDid = DidUtils
            .convertAddressToDid(Keys.getAddress(keyPair));
        credential.setIssuer(keyDid);
    ...
    private ResponseData<CredentialPojo> generateCpt111Credential(
        ...
        args.setIssuer(didAuthentication.getDid());
